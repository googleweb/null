 ...
 *@time
 */
public class ManageCarList_Activity extends BaseActivity {
		...
		try {
			...
			car_login_btu_dl.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					ManageCarList_Activity.this.request("18Q12",
 ...
 */
	...
	protected void dialog_xm() {
		if (!Md_Car_Temp.getTempCar().car_lsh.equals("")) {
			alert("确定退办" + Md_Car_Temp.getTempCar().car_hphm + "车辆 ?",
					new DefautDialog.OnClickListener() {
						@Override
						public void onClick(DefautDialog dialog, View view) {
							dialog.dimiss();
							request("18C72", getRequestData(),
									R.string.DELETE_CAR_INFO,
									new String[] { "1" });
						}
					});
		} else {
			Toast.makeText(ManageCarList_Activity.this, "流水号不能为空！", 2000)
					.show();
		}
	}
	...
	public void onRequestSuccess(String jkid, Object result) {
		...
		if ("18Q12".equals(jkid)) {
			...
			DefautDialog.showDialog(ManageCarList_Activity.this, R.string.SYS_TITLE,
	...
	private void dealCallDeleteCar(List<Md_Car_TongYong> callJS) {
		if (callJS.get(0).code.equals("1")) {
			alert("退办成功！", new DefautDialog.OnClickListener() {
				@Override
				public void onClick(DefautDialog dialog, View view) {
					dialog.dimiss();
					Md_Car_Temp.getTempCar().car_hphm = text_hphm.getText()
	...
	private void dealListMXN(List<MD_Xml_Node> listMXN) {
		...
		if (mDataList != null) {
			cad = new CarBP_Adapter(ManageCarList_Activity.this, mDataList);
		}
		if (cad != null) {
			initListView(cad);
		}
	...
	protected void dialog_alert() {
		DefautDialog.showDialog(
				ManageCarList_Activity.this,
				ManageCarList_Activity.this.getResources().getString(
